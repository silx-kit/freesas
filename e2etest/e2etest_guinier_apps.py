    ...
    def test_csv_output_with_two_files(self):
        ...
        for result in  test_output_result:
            if result[file_name_index] == str(self.bsa_filename):
                ...
                self.assertTrue(float(result[I0_std_index])  < 5, msg=f"I0 std. for BSA by {app_name} below 5")
                self.assertTrue(float(result[first_point_index]) < 70, msg=f"Guinier region start for BSA by {app_name} below 70")
                ...
                self.assertTrue(abs(float(result[Rg_index]) - 3.05) <= 0.2, msg=f"Rg for SASDF52 by {app_name} between 2.85 and 3.25")
                self.assertTrue(float(result[Rg_std_index])  < 0.15, msg=f"Rg std. for SASDF52 by {app_name} below 0.5")
                self.assertTrue(abs(float(result[I0_index]) - 64.5) <= 2.5, msg=f"I0 for SASDF52 by {app_name} between 62 and 67")
                self.assertTrue(float(result[I0_std_index])  < 5, msg=f"I0 std. for SASDF52 by {app_name} below 5")
                self.assertTrue(float(result[first_point_index])  < 35, msg=f"Guinier region start for SASDF52 by {app_name} below 35")
                self.assertTrue(float(result[last_point_index]) > 80, msg=f"Guinier region end for SASDF52 by {app_name} above 80")
